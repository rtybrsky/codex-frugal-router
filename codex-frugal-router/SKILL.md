...
5. 以成本最低且可靠的確定性證據驗證：優先使用 `rg`、diff、現有測試、編譯器、型別檢查、lint、Node/Python 指令或結構化資料檢查。
6. 工具驗證通過且風險中低時直接交付。不要讓另一模型從頭複查同一成果；只有缺乏可靠的自動化驗證，或涉及架構、安全、資料遺失、公開介面時，才做 Sol 的針對性判斷。
